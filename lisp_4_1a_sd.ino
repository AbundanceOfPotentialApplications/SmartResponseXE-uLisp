...
// maybe add status bar with battery, gc, etc
// add guards to SPI operations and enable SD card "bulk" mode
// add lisp extensions for: 
//  floating point math
//  byte-buffers
//  more built in graphics
// 
// add support for the 802.4.15 network, maybe even the zigbee...
// add support for IRDA?
//
// add support for the available IO... like I2C, srxe serial,  LED


// Done:
// add a function for no-delay keyboard read from inside the lisp: getc
// fix the EDIT function
// SLEEP
// add support for SRXE flash
// add support to power-key power down
// better esape way from the inside loop... (power key)
// speaker - note support
//   add built in print-at x,y function, or POSITION x,y: putc

// actually not really needed (ie. possible in lisp itself):
//   read lisp-library/source from SRXE flash
//   ?text editor in parallel or as app...
...
//#include "optiboot.h"                   // not actually mandatory>>
  #define WORKSPACESIZE (2944-SDSIZE)     /* Objects (4*bytes) */ // possibly more?? there's 16k RAM...
  #define EEPROMSIZE 4096                 /* Bytes */
//  #define FLASHWRITESIZE 16384            /* Bytes */ //wut? there's 512 pages, 128words->256 bytes each.what's this size here.
//  #define CODESIZE 96                     /* Bytes <= 256 */
  #define STACKDIFF 320
  #define CPU_ATmega128RFA1

// this should maybe be in the board definition...
  #define TWI_SCL_PIN 0
  #define TWI_SDA_PIN 1
  #define LED_BUILTIN 8 // note: also USB pullup.
...
DACREFERENCE, DELAY, MILLIS, iSLEEP, NOTE, REGISTER, EDIT, PPRINT, PPRINTALL, FORMAT, REQUIRE, LISTLIBRARY,
KEYWORDS, 
...
K_PORTD_IN, K_PORTE_DIR, K_PORTE_OUT, K_PORTE_IN, K_PORTF_DIR, K_PORTF_OUT, K_PORTF_IN,
#endif
USERFUNCTIONS, SCLS, GETC, PUTC, PIXEL, SCROLL, ENDFUNCTIONS, SET_SIZE = INT_MAX };
...
// handling of terminal serial or hardware
int gChar();
void pChar(char t);
bool aChar();
#ifdef hwTerminalWidth
void pCharXY(byte t,int x, int y);
void scls();
#endif
...
#if defined(sdcardsupport)
File SDimagefile;
// Fixme: SD library bamboozled by the copy constructors
//   storing the file as global is a workaround
void SDWriteInt (int data) {
  SDimagefile.write(data & 0xFF); SDimagefile.write(data>>8 & 0xFF);
}

int SDReadInt () {
  uint8_t b0 = SDimagefile.read(); uint8_t b1 = SDimagefile.read();
  return b0 | b1<<8;
}
...
unsigned int saveimage (object *arg) {
  ...
  if (stringp(arg)) {
    ...
    SDimagefile = SD.open(MakeFilename(arg, buffer), O_RDWR | O_CREAT | O_TRUNC);
    if (!SDimagefile) error2(SAVEIMAGE, PSTR("problem saving to SD card or invalid filename"));
    arg = NULL;
  } else if (arg == NULL || listp(arg)) {
    SDimagefile = SD.open("/ULISP.IMG", O_RDWR | O_CREAT | O_TRUNC);
    if (!SDimagefile) error2(SAVEIMAGE, PSTR("problem saving to SD card"));
  }
  else error(SAVEIMAGE, invalidarg, arg);
  SDWriteInt((uintptr_t)arg);
  SDWriteInt(imagesize);
  SDWriteInt((uintptr_t)GlobalEnv);
  SDWriteInt((uintptr_t)GCStack);
  #if defined(CODESIZE)
  for (int i=0; i<CODESIZE; i++) SDimagefile.write(MyCode[i]);
  #endif
  for (unsigned int i=0; i<imagesize; i++) {
    object *obj = &Workspace[i];
    SDWriteInt((uintptr_t)car(obj));
    SDWriteInt((uintptr_t)cdr(obj));
  }
  SDimagefile.close();
  return imagesize;
#elif defined(FLASHWRITESIZE)
...
unsigned int loadimage (object *arg) {
  ...
  if (stringp(arg)) {
    char buffer[BUFFERSIZE];
    SDimagefile = SD.open(MakeFilename(arg, buffer));
    if (!SDimagefile) error2(LOADIMAGE, PSTR("problem loading from SD card or invalid filename"));
  }
  else if (arg == NULL) {
    SDimagefile = SD.open("/ULISP.IMG");
    if (!SDimagefile) error2(LOADIMAGE, PSTR("problem loading from SD card"));
  }
  else error(LOADIMAGE, invalidarg, arg);
  SDReadInt();
  unsigned int imagesize = SDReadInt();
  GlobalEnv = (object *)SDReadInt();
  GCStack = (object *)SDReadInt();
  #if defined(CODESIZE)
  for (int i=0; i<CODESIZE; i++) MyCode[i] = SDimagefile.read();
  #endif
  for (unsigned int i=0; i<imagesize; i++) {
    object *obj = &Workspace[i];
    car(obj) = (object *)SDReadInt();
    cdr(obj) = (object *)SDReadInt();
  }
  SDimagefile.close();
  gc(NULL, NULL);
  return imagesize;
...
void autorunimage () {
  ...
  SD.begin(SDCARD_SS_PIN);
  SDimagefile = SD.open("ULISP.IMG");
  if (!SDimagefile) error2(NIL, PSTR("problem autorunning from SD card"));
  object *autorun = (object *)SDReadInt();
  SDimagefile.close();
  if (autorun != NULL) {
    loadimage(NULL);
...
#if defined(CPU_ATmega4809) || defined(CPU_AVR128DX48) // || defined (SRXE)
...
//#ifdef SRXE
// pins available for PWM:
//6           [PE5:OC3C:INT5] - U5 (unpopulated) pin 5, 
//7   buzzer  [PE3:OC3A:AIN1] - Q1 pin 1 - in dedicated speaker area on PCB
//  if (pin == 6) {
//    DDRE = DDRE | 1 << DDE5;
//    }
//  else 
...
#elif defined(CPU_AVR128DX48)  || defined (SRXE)
  int prescaler = 8 - octave - note/12;
  if (prescaler<0 || prescaler>8) error(NOTE, PSTR("octave out of range"), number(prescaler));
...
object *sp_withsdcard (object *args, object *env) {
  ...
  {
    char buffer[BUFFERSIZE];
    if (SD.remove(MakeFilename(filename, buffer))) return tee;
    return nil;
  }
...
object *fn_sleep (object *args, object *env) {
  (void) env;
  object *arg1 = first(args);
  sleep(checkinteger(iSLEEP, arg1));
  return arg1;
}
...
object *uf_scls(object *args, object *env) {
  (void) args, (void) env;
  scls();
  SRXEScrollReset();
  return nil;
...
object *uf_getc(object *args, object *env) {
  (void) args, (void) env;
  return number(SRXEGetKey());
}
...
object *uf_putc(object *args, object *env) {
  (void) env;
  char str[2];
  int fsize,x,y;
   
  object *arg = third(args);
  str[1]=0;
  if (integerp(arg)) {
    str[0]=checkinteger(PUTC,arg);
  }
  else
  {
    str[0]= checkchar(PUTC,arg);
  }
  x=checkinteger(PUTC,first(args));
  y=checkinteger(PUTC,second(args));
  fsize=FONT_MEDIUM;
  if (car(cdr(cdr(cdr(args)))) != NULL)
  {
    fsize=checkinteger(PUTC,car(cdr(cdr(cdr(args)))));
  }

  if (str[0]>128)
  {
    SRXEWriteString(x,y,str,fsize,0,3);
    }
  else
  {
    SRXEWriteString(x,y,str,fsize,3,0);
  }
...
object *uf_pixel(object *args, object *env) {
  (void) env;
  int x,y;
  byte c;

  x=checkinteger(PIXEL,first(args));
  y=checkinteger(PIXEL,second(args));
  c=checkinteger(PIXEL,third(args));

  SRXESetPosition(x,y,3,1);
  SRXEWriteDataBlock(&c, 1);       
  return nil;
}


object *uf_scroll(object *args, object *env) {
  (void) env;
  int s=0;
  if (first(args)!= NULL) { s=checkinteger(SCROLL,first(args)); }
  return number(SRXEScroll(s));
}
...
const char string_scls[] PROGMEM = "scls";
const char string_getc[] PROGMEM = "getc";
const char string_putc[] PROGMEM = "putc";
const char string_pixel[] PROGMEM = "pixel";
const char string_scroll[] PROGMEM = "scroll";
...
// Insert your own table entries here
  { string10, NULL, 0x00 }, // empty string, to match userfunctions enum 
  { string_scls, uf_scls, 0x00},
  { string_getc, uf_getc, 0x00},
  { string_putc, uf_putc, 0x34},
  { string_pixel, uf_pixel, 0x33},
  { string_scroll, uf_scroll, 0x01},
  { string10, NULL, 0x00 },
};
...
void testescape () {
#ifdef hwTerminalWidth
  if (XgetPowerKey()) error2(NIL, PSTR("escape!"));
#else
  if (gChar() == '~') error2(NIL, PSTR("escape!"));
#endif
}
...
void esc (int p, char c) {
  pChar('\e'); pChar('[');
  pChar((char)('0'+ p/100));
  pChar((char)('0'+ (p/10) % 10));
  pChar((char)('0'+ p % 10));
  pChar(c);
}

void hilight (char c) {
  pChar('\e'); pChar('['); pChar(c); pChar('m');
}
...
#ifdef hwTerminalWidth
if (p)  {
// ')' was just printed at wp-1
// which appears at screenX-1, screenY (or the end of prev line)
// highlight the matching '(' at p...
// p is actually the distance from '(' to the wp.


//  wp+=screenX+screenY*hwTerminalWidth - 1;
  p =screenX+screenY*hwTerminalWidth-p; // - wp;
  //wp+=screenX-1+screenY*hwTerminalWidth - wp;
  wp =screenX-1+screenY*hwTerminalWidth;

  //int col= ((screenX>0)?(screenX-1):(hwTerminalWidth-1));
  //int line= ((screenX>0)?screenY:(screenY-1));
  //p=wp-p-1+screenX+screenY*hwTerminalWidth;
  ...
  pCharXY('('+(invert?128:0),p%hwTerminalWidth,p/hwTerminalWidth);
  pCharXY(')'+(invert?128:0),wp%hwTerminalWidth,wp/hwTerminalWidth);
    //((screenX>0)?(screenX-1):(hwTerminalWidth-1)),
    //((screenX>0)?screenY:(screenY-1)));
}
#else       
  wp = wp + 2; // Prompt
...
void processkey (char c) {
...
#if defined(vt100) || defined(hwTerminalWidth)
  ...
  if (c == ')') {
    ...
    while (search >= 0 && parenthesis == 0) {
      ...
      if (c == ')') level++;
      if (c == '(') {
        level--; // ie. ( at 3 ) at 5... wp=6, seach = 2, p=wp-s-1 = 3
...
#else
  unsigned long start = millis();
  while (!aChar()) if (millis() - start > 1000) clrflag(NOECHO);
  char temp = gChar();
...
void setup () {
  ...
  SRXEScrollArea(0,128,32);
  scls();
  //pchar('P');pchar('o');pchar('w');pchar('e');pchar('r');pchar('i');pchar('n');pchar('g');pchar(' ');
  //pchar('U');pchar('P');pchar('.'); pln(pserial);
  pfstring(PSTR("Power... up..."), pserial);
  long int start = millis();
  while ((millis() - start) < 1000) ; // just wait 1 second to stabilize the flash chip
  scls();  
#else
  Serial.begin(9600); 
  int start = millis();
...
// FONT_NORMAL FONT_SMALL FONT_MEDIUM FONT_LARGE 3

// y scroll is compensated for in the library...
void pCharXY(byte t, int x, int y)
{
  pCharV[0]=(char)(t & 0x7f);
  SRXEWriteString(x*12,y*16,pCharV,FONT_MEDIUM,((t&0x80)?0:3),((t&0x80)?3:0));
}
#endif
...
void QDebug(char t)
{
  ...
  pCharV[0] = '0'+t%10;
  SRXEWriteString(t*6,128,pCharV,FONT_SMALL,3,0);
  
  if(t>10)
  {
    pCharV[0] = '0'+(t/10)%10;
    SRXEWriteString((t-1)*6,128,pCharV,FONT_SMALL,3,0);
  }
...
void pChar(char t)
{
  ...
  switch(t)
  {
    ...
    case 8: screenX--; // delete
            break;
    case '\r': break;
    case '\n':  // new line,cr,lf
            screenX=0; screenY++; break;
    default:
            pCharXY(t,screenX,screenY);
            screenX++;
  }
  
  if (screenX>=hwTerminalWidth)
   {
     screenX=0; screenY++; 
   }
  ...
  {
    screenY--;
    screenX=hwTerminalWidth-1;
  }
  
 while (screenY>=8)
  {
    //clear first row
    SRXERectangle(0, 0, 128, 16, 0, 1);
    SRXEScroll(16);
    screenY--;  
  }
 while (screenY<0)
  {
    //clear last row
    SRXERectangle(0, 112, 128, 16, 0, 1);
    screenY++;  
    SRXEScroll(-16);
  }
...
{
#ifdef hwTerminalWidth
  if (aCharV) { aCharV = 0; return aCharC; }
  aCharC = SRXEGetKey();
  if (aCharC == 0) return -1;
// powerCheck(1);
  return aCharC; 
#else
  return Serial.read();
...
// powerCheck(0);
  if (aCharV) return true;  
  aCharC = SRXEGetKey();
  if (aCharC == 0) return false;
  aCharV = true;  
#else
...
void powerCheck(char activity)
{
  ...
  static int autoPower, buttonPower;
  uint16_t newTime;
  ...
  if (oldTime != newTime)
  {
    oldTime = newTime;
    ++autoPower;
    if (XgetPowerKey()) buttonPower++;
      else buttonPower = 0;
  }

  if (activity) autoPower = 0;
  
  if ((buttonPower>36) || (autoPower>2400))
  {
    SRXESleep(); // includes powerdown and power up for the display...
    SRXEScrollArea(0,128,32);
    scls(); //
    ...
    for(newTime = 0;newTime<WritePtr; newTime++)
      pChar(KybdBuf[newTime]);
