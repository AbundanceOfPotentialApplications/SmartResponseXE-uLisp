...
//  fdufnews 12/2019
//
static unsigned char scrollArea=160;
static unsigned char scrollStart=0;
...
static int scrollY(int y)
{
  if ((y>=scrollStart)  &&
      (y<scrollStart+scrollArea))
  {
    return scrollStart+(y-scrollStart+iScrollOffset)%scrollArea;    
  }
  return y;
}
...
byte bTemp[128];
  y=scrollY(y);
   ...
   // cx is width in 3 pixel blocks.
   // 3x+3cx-1 is the right-most individual pixel written.
   // (that 1 is subtracted in the SetPosition function)
   // and THAT pixel needs to be no larger than 128*3-1

   if (bFilled)
   {
      SRXESetPosition(x*3, y, cx*3, cy);  // goes from x/3 to (x+cx-1)/3
      // so, x=0, cx=127 goes to:  380/3 = 126
      for (y=0; y<cy; y++)
      {
